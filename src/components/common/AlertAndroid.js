...
class AlertAndroid extends Component {
    ...
    render() {
        return(
            ...
            this.state.isShow &&
            <Animated.View style={{height:deviceHeight,width:deviceWidth,justifyContent:'center',alignItems:'center',backgroundColor:'rgba(0,0,0,0.4)',position:'absolute',top:-50,opacity: this.state.opacityValue}}>
                <Animated.View style={{width:deviceWidth / 6 * 4,transform: [{scale:this.state.scaleValue}]}}>
                    <View style={{height:this.state.options.length > 2 ? 70 : 75,borderTopLeftRadius:10,borderTopRightRadius:10,justifyContent:'center',alignItems:'center',backgroundColor:'white'}}>
                        <Text style={{fontSize:16,color:'#333333'}}>{this.state.alertText ? '提示' : this.state.title}</Text>
                        <Text style={{fontSize:14,color:'#AEAEAE',marginTop: 6}}>{this.state.alertText ? this.state.alertText : this.state.detailText}</Text>
                    </View>
